...
def test_whisper():
    try:
        ...
        audio_file = "../whisper.cpp/samples/jfk.wav"  # Replace with your audio file path
        ...
        result = whisper_full(ctx, params, audio_data_p, ctypes.c_int(len(audio_data)))
        ...
        if 'ctx' in locals():
            try:
                logging.info("Attempting to free Whisper context")
                whisper.whisper_free(ctx)
                logging.info("Whisper context freed successfully")
            except Exception as e:
                logging.error(f"Error freeing Whisper context: {e}")
